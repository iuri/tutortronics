...
ad_proc -public im_indicator_section_helpdesk {}	{ return 15255 }
ad_proc -public im_indicator_section_absences {}	{ return 15260 }
ad_proc -public im_indicator_section_confdb {}	        { return 15265 }
...
ad_proc im_indicator_timeline_widget_sencha {
    -name
    -report_id
    { -initial_date "" }
    { -periodic_where "" }
    ...
    Returns a ExtJS Chart
} {

    set params [list \
		    [list report_id $report_id] \
		    [list initial_date $initial_date] \
		    [list periodic_where $periodic_where] \
		    [list start_date $start_date ] \
		    [list end_date $end_date] \
		    [list x_coordinate "x_coordinate"] \
		    [list y_coordinate "y_coordinate"] \
		    [list diagram_width $diagram_width] \
		    [list diagram_height $diagram_height] \
		    [list render_to $render_to] \
		    [list title $title] \
		    [list label_x $label_x] \
		    [list label_y $label_y] \
		    [list dateFormat $dateFormat] \
		    ]

    set result [ad_parse_template -params $params "/packages/intranet-reporting-indicators/lib/single-timeline"]
    set result "<script type=\"text/javascript\">$result</script>"
...
ad_proc -public im_indicator_evaluation_sweeper { 
    ...
    set current_user_id [db_string default_user "select  min(person_id) from persons where person_id > 0"]
...
ad_proc -public im_indicator_timeline_component { 
    ...
    set wiki_url "http://www.project-open.com/en"
    set package_url "/intranet-reporting-indicators"

    if {"" eq $start_date} { set start_date [db_string start "select (now()-'12 months'::interval)::date"] }
    if {"" eq $end_date} { set end_date [db_string start "select now()::date"] }


    set indicator_timeline_max_days_backwards_msg_default "Current configuration does not allow a start date earlier than %initial_date% for this indicator"
    set indicator_timeline_max_days_backwards_msg [lang::message::lookup "" intranet-reporting-indicators.Max_Days_Set $indicator_timeline_max_days_backwards_msg_default]
    ...
    db_multirow -extend {report_view_url edit_html value_html history_html indicator_timeline_max_days_backwards_msg} reports get_reports "
	select
		r.*,
		i.*,
		im_category_from_id(i.indicator_section_id) as section
	...
	order by 
		section
    " {
	set report_view_url [export_vars -base "$package_url/view" {indicator_id return_url}]
	set report_edit_url [export_vars -base "$package_url/new" {indicator_id}]
	set perms_url [export_vars -base "$package_url/perms" {{object_id $indicator_id}}]
	set delete_url [export_vars -base "$package_url/delete" {indicator_id return_url}]
	set edit_html "
	...
	regsub -all " " $report_name "-" indicator_name_mangled
	set help_url "$wiki_url/indicator-[string tolower $indicator_name_mangled]"
	...
	set indicator_timeline_max_days_backwards_msg ""
	...
	set history_html "<span id='diagram_container_$report_id'>"
	append history_html [im_indicator_timeline_widget_sencha \
                      -report_id $report_id \
		      -name $report_name \
		      ...
		      -diagram_height 200 \
                      -title $title \
                      -render_to "diagram_container_$report_id" \
                      -label_x $label_x \
                      -label_y $label_y \
                      -dateFormat $dateFormat \
	]
	append history_html "</span>"
    }
	...
	section {
	    label "#intranet-reporting-indicators.Section#"
	    display_template {
		@reports.section@
	    }
	}
    ...
    lappend elements_list \
	name {
	    label "#intranet-reporting-indicators.Name#"
	    display_template {
    ...
    if {0 != $show_current_value_p} {
       lappend elements_list \
	value {
	    label "#intranet-reporting-indicators.Value#"
	    display_template {
    ...
    lappend elements_list \
	history {
	    label "#intranet-reporting-indicators.History#"
	    display_template {
   ...
   if {0 != $show_description_long_p} {
        lappend elements_list \
	report_description {
	    label "#intranet-reporting-indicators.Description#"
	    display_template {
		@reports.report_description;noquote@</br>
		@reports.indicator_timeline_max_days_backwards_msg;noquote@
	    }
	}
    }
...
ad_proc -public im_indicator_home_page_component { 
    ...
    set params [list \
	[list user_id [ad_conn user_id]] \
	[list module $module] \
	[list return_url [im_url_with_query]] \
    ]

    if { "" eq [im_sencha_extjs_version] } {
	set result [ad_parse_template -params $params "/packages/intranet-reporting-indicators/www/indicator-home-component"]
    } else {
	set result [ad_parse_template -params $params "/packages/intranet-reporting-indicators/lib/indicator-home-component-gauge"]
    }
