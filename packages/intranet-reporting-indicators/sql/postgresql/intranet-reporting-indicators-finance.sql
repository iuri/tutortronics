...
',
	...
	update im_reports set
		report_description = 'Counts how many financial documents ' ||
		'(quotes, pos, invoices, bills, deliver notes or expense invoices) ' ||
		'have been generated for all main projects that ended (end_date) 30 days ago to 60 days ago.'
...
BEGIN
	v_id := im_indicator__new(
		...
		15110,
		15000,
		'select coalesce(round(sum(amount),0),0) from im_costs where cost_type_id = 3704 and effective_date between now()::date-60 and now()::date-30',
		0,
		1000,
		5
	);
