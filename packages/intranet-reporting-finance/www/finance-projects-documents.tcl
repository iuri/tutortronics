...
Report provides a reasonable approximation of profitability in a given period. It is particullary helpful in organizations that deal with a large number of smaller projects.<br>
<ul>
	<li>Report includes all projects with an end date that lie between the start date and end as defined in the report filters.</li>
	<li>Report shows <u>all</u> financial documents for the selected projects, even if their creation and due dates are outside of the period as set in the report filters (start date/end date).</li>
	<li>Projects and tasks require a valid start date and end date in order to be considered in this report</li>
	<li>Start date is inclusive (document with effective date = start date or later), while end date is exclusive</li>
	<li>In opposite to other reports, project name is shown on the bottom of each group in this report</li>
	<li>Fields:
		<ul>
		<li>Customer: Customer name</li>
		<li>Project: Only main projects are shown</li>
		<li>Sales Representive: Sales Representative (creator of the financial document)</li>
		<li>Invoice: Financial documents of type 'Invoice', independent from their status</li>
		<li>DelNote: Financial documents of type 'Delivery Note', independent from their status</li>
		<li>Quote: Financial documents of type 'Quote, independent from their status</li>
		<li>Bill: Financial documents of type 'Bill', independent from their status</li>
		<li>PO: Financial documents of type 'Purchase Order', independent from their status</li>
		<li>Expense: Amount of Expense Bundles</li>
		<li>Timesheet: Logged hours multiplied by hourly rate found when time has been logged</li>
		<li>PO/Quote: Ratio btw. Purchase Orders and Quotes (in %)</li>
		<li>Gross Profit: Invoices - Quotes - Expenses</li>
		<li>WIP: Work in Progress: Timesheets + Bills + Expenses - Invoices
		</ul>
	</li>
</ul>
...
select
	...
	to_char(p.project_budget, :cur_format) as project_budget_formatted,
...
db_foreach sql $sql {
    ...
    if {[expr $quote_subsubtotal+0] != 0} {
        # ns_log Notice "finance-projects-documents po_subsubtotal: $po_subsubtotal, quote_subsubtotal: $quote_subsubtotal"
        set po_per_quote_perc_subsubtotal [expr int(10000.0 * $po_subsubtotal / $quote_subsubtotal) / 100.0]
        # ns_log Notice "finance-projects-documents po_per_quote_perc_subsubtotal: $po_per_quote_perc_subsubtotal"
        set po_per_quote_perc_subsubtotal "$po_per_quote_perc_subsubtotal %"
        # ns_log Notice "finance-projects-documents po_per_quote_perc_subsubtotal: $po_per_quote_perc_subsubtotal"
        # ns_log Notice "finance-projects-documents ---"
    }
