...
set main_project_header {
	"" 
	"\#colspan=13 <nobr><a href=$this_url&main_project_id=$main_project_id&level_of_detail=4 
	target=_blank><img src=/intranet/images/plus_9.gif width=9 height=9 border=0></a>
...
set report_def [list \
    ...
    header {
	"\#colspan=18 <a href=$this_url&customer_id=$project_customer_id&level_of_detail=4 
	target=_blank><img src=/intranet/images/plus_9.gif width=9 height=9 border=0></a> 
...
db_foreach sql $sql {
	...
	set po_per_quote_perc_subsubtotal ""
	...
	set gross_profit_subsubtotal [format "%.2f" [expr {double(round(100*$gross_profit_subsubtotal))/100}]]

	set wip_subsubtotal [expr $timesheet_subsubtotal + $bill_subsubtotal + $expense_subsubtotal - $invoice_subsubtotal]
	set wip_subsubtotal [format "%.2f" [expr {double(round(100*$wip_subsubtotal))/100}]]

	# Calculated Variables for footer0
	set po_per_quote_perc_subtotal ""
	...
	set gross_profit_subtotal [format "%.2f" [expr {double(round(100*$gross_profit_subtotal))/100}]]

	set wip_subtotal [expr $timesheet_subtotal + $bill_subtotal + $expense_subtotal - $invoice_subtotal]
        set wip_subtotal [format "%.2f" [expr {double(round(100*$wip_subtotal))/100}]]

	set timesheet_total [format "%.2f" [expr {double(round(100*$timesheet_total))/100}]]
	set timesheet_subtotal "[format "%.2f" [expr {double(round(100*$timesheet_subtotal))/100}]]"
	set timesheet_subsubtotal "[format "%.2f" [expr {double(round(100*$timesheet_subsubtotal))/100}]]"
...
set gross_profit_total [format "%.2f" [expr {double(round(100*$gross_profit_total))/100}]]

set wip_total [expr $timesheet_total + $bill_total + $expense_total - $invoice_total]
set wip_total [format "%.2f" [expr {double(round(100*$wip_total))/100}]]
