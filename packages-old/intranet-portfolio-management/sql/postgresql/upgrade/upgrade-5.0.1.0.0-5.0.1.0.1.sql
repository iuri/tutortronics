...
BEGIN
	...
	update im_projects set score_roi = project_roi;
	update acs_attributes set attribute_name = 'score_roi' where attribute_name = 'project_roi' and table_name = 'im_projects';
	alter table im_projects drop column project_roi;

	alter table im_projects add column score_strategic_value numeric(12,1);
	update im_projects set score_strategic_value = project_strategic_value;
	update acs_attributes set attribute_name = 'score_strategic_value' where attribute_name = 'project_strategic_value' and table_name = 'im_projects';
