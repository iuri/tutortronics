...
select distinct
	...
	(select external_company_vat_number from im_expenses exp where exp.expense_id = c.cost_id) as expense_vat_number,
        ...
        c.project_id as cost_project_id,
        (select project_nr from im_projects p where c.project_id = p.project_id) as cost_project_nr,
        (select project_name from im_projects p where c.project_id = p.project_id) as cost_project_name,
        (select min(object_id_one) from acs_rels where object_id_two = c.cost_id) as rel_project_id,
        (select pp.project_nr from im_projects pp where project_id in (select min(object_id_one) from acs_rels where object_id_two = c.cost_id)) as rel_project_nr,
        (select pp.project_name from im_projects pp where project_id in (select min(object_id_one) from acs_rels where object_id_two = c.cost_id)) as rel_project_name
from
	im_costs c
...
set report_def [list \
    ...
    header {
	"\#colspan=12 Quarter $effective_year_quarter"
    } \
        content [list \
            group_by cost_type \
            header { 
	    ...
	    content [list \
		    ...
		    content [list \
				...
				"<nobr><a href=/intranet/projects/view?project_id=$project_id>$project_name</a></nobr>"
			    } \
			    content {} \
		    ] \
	            footer {
...
set header0 {"Q" "Cost<br>Type" "VAT<br>Type" "Customer" "VAT<br>Number" "Effective<br>Date" "Name" "Amount" "Vat" "Tax" "Project"}
set footer0 {
	"" 
	"" 
	"" 
...
db_foreach sql $sql {
    ...
    # Extract project per cost item
    set project_id $cost_project_id
    set project_name $cost_project_name
    if {"" != $rel_project_id} {
        set project_id $rel_project_id
        set project_name $rel_project_name
    }
